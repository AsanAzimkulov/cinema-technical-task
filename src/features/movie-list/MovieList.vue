<template>
  <div class="space-y-6">
    <div class="text-center">
      <h1 class="text-3xl font-bold text-gray-900">Фильмы</h1>
      <p class="mt-2 text-lg text-gray-600">Выберите фильм для просмотра расписания сеансов</p>
    </div>
    ...
    <div v-else class="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
      <MovieCard
        ...
        :key="movie.id"
        :movie="movie"
        @view-sessions="navigateTo(`/movies/${movie.id}`)"
      />
    </div>
  </div>
</template>
